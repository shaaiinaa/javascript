...
const arr = [];
for (let value of data.recipes) {
  arr.push({ arrName: value.name, arrCuisine: value.cuisine });
}
let count = 0;
document.write(`<table><tr><th>Name</th><th>Cuisine</th></tr>`);
for (let i = 0; i < arr.length; i++) {
  count++;
  document.write(
    `<tr><td>${arr[i].arrName}</td><td> ${arr[i].arrCuisine}</td>`
  );
  console.log(arr[i].arrName, arr[i].arrCuisine);
}
