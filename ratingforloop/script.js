...
for (let value of data.recipes) {
  arr.push({
    arrName: value.name,
    arrCuisine: value.cuisine,
    arrRating: value.rating,
  });
}
let count = 0;
for (let i = 0; i < arr.length; i++) {
  if (arr[i].arrRating > 4.5) {
    count++;
    document.write(`<h3>${arr[i].arrName}: ${arr[i].arrCuisine}</h3>`);
  }
}

document.write(`<h2>Total Recipes: ${count}</h2>`);
