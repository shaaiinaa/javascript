...
//   (Id = data.recipes[i].id), (Name = data.recipes[i].name);

//   document.write(
//     `<div id="data-card" style=" border: 1px solid black;
//     padding: 10px;
//     margin: 20px;
//     text-align: center;
//     background:no-repeat center/100% url(${data.recipes[i].image} );
//     color:beige;
//     margin:20px auto ;
...
//     height: 350px;
//     border-radius: 10px;
//      box-shadow: 0 0 3px 3px #48abe0;

//   ">` +
//       `<h2 >${Name}</h2>` +
//       `<button style="background:red; padding:10px;" onclick="divDelete()">delete</button>` +
//       `<p>Difficulty: ${data.recipes[i].difficulty}</p>` +
//       `<p>Cuisine: ${data.recipes[i].cuisine}</p>` +
//       `<p>Servings: ${data.recipes[i].servings}</p>` +
//       `<p>Rating: ${data.recipes[i].rating}</p>` +
//       `<p>Review Count: ${data.recipes[i].reviewCount}</p>` +
//       `<p>Meal Type: ${data.recipes[i].mealType}</p>` +
//       `<p>Prep Time: ${data.recipes[i].prepTimeMinutes} minutes</p>` +
//       `<p>Cook Time: ${data.recipes[i].cookTimeMinutes} minutes</p>` +
//       `<p>Calories Per Serving: ${data.recipes[i].caloriesPerServing}</p>` +
//       `<p>Tags: ${data.recipes[i].tags}</p>` +
//       `<p>User ID: ${data.recipes[i].userId}</p>` +
//       `</div>`
//   );
// }
// function divDelete() {
//   const abc = document.getElementById("data-card");
//   abc.remove();
// }
...
data.recipes.forEach((recipe) => {
  ...
  card.className = "card";
  card.style.background = `no-repeat center/cover url(${recipe.image})`;

  card.innerHTML = `
    <div id="main1" class="trial">
        <h2>${recipe.name}</h2>
        <p>Rating:${recipe.rating}</p>
        <p>Cuisine:${recipe.cuisine}</p>
        <p>Meal Type:${recipe.mealType}</p>
        <button onclick="deleteDiv()">delete</button>
         <p id="info-${recipe.id}" onclick="showMore(event)">SHOW MORE</p>
         <div id="more-info-${recipe.id}" style="display: none;">
           ...
           <p class="instructions" title="${recipe.instructions}">${
    recipe.instructions
  }</p>
            <p
...
function showMore(e) {
  ...
  const moreInfo = document.getElementById(`more-${currId}`);
  document.getElementById(currId).style.display = "none";
  moreInfo.style.display = "block";
  document.getElementById("main1").style.width = "600px";
}
function showLess() {
...
function deleteDiv() {
  const abc = document.getElementById("main1");
  abc.remove();
}
