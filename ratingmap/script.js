...
//     // return [recipe.name, recipe.cuisine].join(", ");
//     document.write(`<h3>${recipe.name}: ${recipe.cuisine}</h3>`);
//   }
// });
// document.write(`${count}`);
const arr = data.recipes.map((recipe) => {
  return {
    name: recipe.name,
...
let ratings = arr.filter((abc) => abc.rating > 4.5);
for (let i = 0; i < ratings.length; i++) {
  //   if (ratings[i].rating > 4.5) {
  document.write(`<h3>${ratings[i].name}: ${arr[i].cuisine}</h3>`);
  // }
}
