...
const addButton = (card, text, className, idx) => {
  ...
  btn.addEventListener("click", () => deleteRecipe(idx));
  card.appendChild(btn);
};
const deleteRecipe = (idx) => {
  recipes.splice(idx, 1);
  rootParent.textContent = "";
  createUI(recipes);
};
const createCard = (
  container,
  { id, name, servings, cuisine, difficulty },
  idx
) => {
  ...
  addPara(card, id);
  addPara(card, name);
  addPara(card, servings);
  addPara(card, cuisine);
  addPara(card, difficulty);
...
const createUI = (arr) => {
  ...
  rootParent.appendChild(container);
};
createUI(recipes);
